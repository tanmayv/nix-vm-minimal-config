...

{
  ...
  systemd.services.nixos-upgrade-from-metadata = {
    ...
    path = [ pkgs.curl pkgs.nix pkgs.openssh pkgs.gnugrep pkgs.coreutils ];
    ...
    script = ''
      ...
      echo "New system found: $TARGET_SYSTEM"
  ...
  systemd.services.nixos-generation-to-metadata = {
    ...
    script = ''
      ...
      while read -r line; do
        ...
        # Add the entry to our JSON array
        JSON_PAYLOAD=$(echo "$JSON_PAYLOAD" | jq --argjson entry "$json_entry" '. + [$entry]')
        
      done <<< "$GENERATIONS_INFO"

      # Write the generations data to guest attributes
      curl -f -H "Metadata-Flavor: Google" \
        -X PUT --data-binary "$JSON_PAYLOAD" \
        http://metadata.google.internal/computeMetadata/v1/instance/guest-attributes/vm_generations
    '';
    serviceConfig = {
      Type = "oneshot";
      User = "root";
